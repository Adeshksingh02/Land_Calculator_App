...
if option == "फीट (Feet) से नापें":
    ...
    with col1:
        # min_value हटा दिया गया है ताकि -1 डालने पर UI न रुके
        l1 = st.number_input("पूर्व (East) की लंबाई - फीट में", format="%.2f", key="f_l1")
        w1 = st.number_input("उत्तर (North) की चौड़ाई - फीट में", format="%.2f", key="f_w1")
    with col2:
        l2 = st.number_input("पश्चिम (West) की लंबाई - फीट में", format="%.2f", key="f_l2")
        w2 = st.number_input("दक्षिण (South) की चौड़ाई - फीट में", format="%.2f", key="f_w2")

    if st.button("कैलकुलेट करें (Calculate Feet)"):
        # चारों भुजाओं की वैलिडेशन (Validation)
        if l1 > 0 and l2 > 0 and w1 > 0 and w2 > 0:
            ...
            st.error("❌ गलती: कृपया चारों भुजाओं में 0 से बड़ी संख्या भरें! (Negative या 0 मान्य नहीं है)")
...
# ==========================================
    ...
    col1, col2 = st.columns(2)
    with col1:
        l1 = st.number_input("पूर्व (East) की लंबाई - लाठा में", format="%.2f", key="l_l1")
        w1 = st.number_input("उत्तर (North) की चौड़ाई - लाठा में", format="%.2f", key="l_w1")
    with col2:
        l2 = st.number_input("पश्चिम (West) की लंबाई - लाठा में", format="%.2f", key="l_l2")
        w2 = st.number_input("दक्षिण (South) की चौड़ाई - लाठा में", format="%.2f", key="l_w2")
...
# ==========================================
    ...
    st.markdown("खेत की चारों भुजाओं की लंबाई **मीटर** में डालें:")
